...
import NurseryDetails from "@/pages/polyhouse/polyhouseDetails/Components/Nursery/nurseryDetails";
import NurserySensor from "@/pages/polyhouse/polyhouseDetails/Components/Nursery/nurserySensor";
import routePaths from "@/config/routePaths";
import NurseryComponent from "@/pages/polyhouse/polyhouseDetails/Components/Nursery/nurseryComponentData";
...
describe("nursery details page", () => {
  ...
  test("should render the nursery details component correctly", () => {
    ...
    renderWithProvider(<NurseryDetails />, { store });
  ...
  test("should render error state", () => {
    store = setupDefaultStore({
      error: {
        [PolyhouseActions.REQUEST_NURSERY_FINISHED]: {
  ...
  test("should request nursery when selectedNursery is null", async () => {
    store = setupDefaultStore({
      polyhouses: { ...dummyPolyhouse, selectedNursery: null },
      farms: { ...dummyFarms },
      users: { ...dummyUser },
    });
    renderWithProvider(<NurseryDetails />, { store });
    ...
    await waitFor(() => {
      expect(store.dispatch).toHaveBeenCalledWith(
        PolyhouseActions.requestNursery("ple11ad963", "nub4cde909")
      );
    });
  });
});

describe("Nursery sensors", () => {
  let store: any;
  ...
  test("should render nursery sensor correctly", () => {
    store = setupDefaultStore({
      farms: { ...dummyFarms },
      polyhouses: { ...dummyPolyhouse },
      users: { ...dummyUser },
    });
    renderWithProvider(<NurserySensor />, { store });

    expect(screen.getByText("CO2")).toBeInTheDocument();
    expect(screen.getByText("8 PPM")).toBeInTheDocument();
  });

  test("should dispatch requestNurserySensorData action on refresh button click", () => {
    store = setupDefaultStore({
      farms: { ...dummyFarms },
      polyhouses: { ...dummyPolyhouse },
    });
    ...
    expect(store.dispatch).toHaveBeenCalledWith(
      PolyhouseActions.requestNurserySensorData("nub4cde909")
    );
  });

  test("should navigate to nursery sensor details page on sensor click", () => {
    store = setupDefaultStore({
      farms: { ...dummyFarms },
      polyhouses: { ...dummyPolyhouse },
    });
    ...
    expect(mockNavigate).toHaveBeenCalledWith(
      routePaths.nurserySensorDetails
  ...
  test("should display message when no sensor is available", () => {
    store = setupDefaultStore({
      farms: { ...dummyFarms },
      polyhouses: { ...dummyPolyhouse, nurserySensorData: null },
    });
    renderWithProvider(<NurserySensor />, { store });
...
describe("nursery component", () => {
  ...
  test("should render nursery component correctly", () => {
    store = setupDefaultStore({
      farms: { ...dummyFarms },
      polyhouses: { ...dummyPolyhouse },
      users: { ...dummyUser },
    });
    renderWithProvider(<NurseryComponent />, { store });
  ...
  test("should dispatch requestData action on refresh button click", async () => {
    ...
    await waitFor(() => {
      expect(store.dispatch).toHaveBeenCalledWith(
        PolyhouseActions.requestNurseryComponentData("nub4cde909")
      );
    });
  });
  ...
  test("should display message when no component is available", () => {
    store = setupDefaultStore({
      farms: { ...dummyFarms },
      polyhouses: { ...dummyPolyhouse, nurseryComponentData: null },
    });
    renderWithProvider(<NurseryComponent />, { store });
