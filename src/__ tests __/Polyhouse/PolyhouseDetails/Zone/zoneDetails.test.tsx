...
import ZoneDetails from "@/pages/polyhouse/polyhouseDetails/Components/Zones/zoneDetails";
import PolyhouseActions from "@/redux/polyhouse/action";
import ZoneSensor from "@/pages/polyhouse/polyhouseDetails/Components/Zones/zoneSensor";
import routePaths from "@/config/routePaths";
import ZoneComponent from "@/pages/polyhouse/polyhouseDetails/Components/Zones/zoneComponent";
...
describe("zone details page", () => {
  ...
  test("should render the zone details component correctly", () => {
    ...
    renderWithProvider(<ZoneDetails />, { store });
  ...
  test("should render ZoneDetails loader when api request is send", async () => {
    store = setupDefaultStore({
      farms: { ...dummyFarms },
      polyhouses: { ...dummyPolyhouse },
      users: { ...dummyUser },
      requesting: {
        ...
        [PolyhouseActions.REQUEST_ZONE]: true,
      },
      tasks: {
        tasks: {
          all: {
  ...
  test("should render error state", () => {
    store = setupDefaultStore({
      farms: { ...dummyFarms },
      error: {
        [PolyhouseActions.REQUEST_ZONE_FINISHED]: {
  ...
  test("should request zone when selectedZone is null", async () => {
    store = setupDefaultStore({
      polyhouses: { ...dummyPolyhouse, selectedZone: null },
      farms: { ...dummyFarms },
      users: { ...dummyUser },
    });
    renderWithProvider(<ZoneDetails />, { store });
    ...
    await waitFor(() => {
      expect(store.dispatch).toHaveBeenCalledWith(
        PolyhouseActions.requestZone("ple11ad963", "zn41f063fb")
      );
    });
  });
});

describe("Zone sensors", () => {
  let store: any;
  ...
  test("should render zone sensor correctly", () => {
    store = setupDefaultStore({
      farms: { ...dummyFarms },
      polyhouses: { ...dummyPolyhouse },
      users: { ...dummyUser },
    });
    renderWithProvider(<ZoneSensor />, { store });

    expect(screen.getByText("CO2")).toBeInTheDocument();
    expect(screen.getByText("8 PPM")).toBeInTheDocument();
  });

  test("should dispatch requestZoneSensorData action on refresh button click", () => {
    store = setupDefaultStore({
      farms: { ...dummyFarms },
      polyhouses: { ...dummyPolyhouse },
    });
    ...
    expect(store.dispatch).toHaveBeenCalledWith(
      PolyhouseActions.requestZoneSensorData("zn41f063fb")
    );
  });

  test("should navigate to zone sensor details page on sensor click", () => {
    store = setupDefaultStore({
      farms: { ...dummyFarms },
      polyhouses: { ...dummyPolyhouse },
    });
  ...
  test("should display message when no sensor data is available", () => {
    store = setupDefaultStore({
      farms: { ...dummyFarms },
      polyhouses: { ...dummyPolyhouse, zoneSensorData: null },
    });
    renderWithProvider(<ZoneSensor />, { store });
...
describe("Zone component", () => {
  ...
  test("should render zone component correctly", () => {
    store = setupDefaultStore({
      farms: { ...dummyFarms },
      polyhouses: { ...dummyPolyhouse },
      users: { ...dummyUser },
    });
    renderWithProvider(<ZoneComponent />, { store });
  ...
  test("should dispatch requestData action on refresh button click", async () => {
    ...
    await waitFor(() => {
      expect(store.dispatch).toHaveBeenCalledWith(
        PolyhouseActions.requestZoneComponentData("zn41f063fb")
      );
    });
  });
  ...
  test("should display message when no component is available", () => {
    store = setupDefaultStore({
      farms: { ...dummyFarms },
      polyhouses: { ...dummyPolyhouse, zoneComponentData: null },
    });
    renderWithProvider(<ZoneComponent />, { store });
