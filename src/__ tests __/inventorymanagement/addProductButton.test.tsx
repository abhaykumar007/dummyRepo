...
import AddProductButton from "@/pages/inventory/addInventory/addProductButton";
...
describe("Add  Product Button", () => {
  ...
  it("should render the add product button", () => {
    renderWithProvider(<AddProductButton />, { store });
  ...
  it("should give error on submit without filling the form", async () => {
    renderWithProvider(<AddProductButton />, { store });
    const button = screen.getByRole("button", { name: /Add Product/i });
    await userEvent.click(button);
    const okButton = screen.getAllByRole("button", { name: /Add/i });
    await userEvent.click(okButton[1]);
    await waitFor(() => {
      ...
      expect(screen.getByText("Please select unit")).toBeInTheDocument();
    });
  });

  it("should call create product action on submit", async () => {
    renderWithProvider(<AddProductButton />, { store });
    const button = screen.getByRole("button", { name: /Add Product/i });
    await userEvent.click(button);
    ...
    const unitSelect = screen.getByTestId("unit-select").firstElementChild;
    if (unitSelect) {
      await userEvent.click(unitSelect);
      const unitOption = screen.getAllByText("Nos");
      await userEvent.click(unitOption[1]);
    }
    const okButton = screen.getAllByRole("button", { name: /Add/i });
    await userEvent.click(okButton[1]);
    expect(store.dispatch).toHaveBeenCalledWith(
      InventoryActions.createProduct({
        name: "Test Product",
        subCategoryId: "65dcc0591e82ab3fc6d9d768",
        unit: "Nos",
      })
  ...
  it("should disable add button on loading", async () => {
    store = setupDefaultStore({
      inventories: {
        inventories: {
          entities: {
      ...
      requesting: {
        [InventoryActions.CREATE_PRODUCT]: true,
      },
    });
    renderWithProvider(<AddProductButton />, { store });
    const button = screen.getByRole("button", { name: /Add Product/i });
  ...
  it("should show error message on create product failure", async () => {
    store = setupDefaultStore({
      inventories: {
        inventories: {
          entities: {
      ...
      error: {
        [InventoryActions.CREATE_PRODUCT_FINISHED]: {
          errors: [
            {
              error: "Undefined",
