...
describe("Inventory Table", () => {
  ...
  it("should call inventory actions on first render", () => {
    store = setupDefaultStore({
      inventories: {
        inventories: {
          entities: {},
    ...
    renderWithProvider(<InventoryTable />, { store });
    expect(store.dispatch).toHaveBeenCalledWith(
      InventoryActions.fetchInventories()
    );
    expect(store.dispatch).toHaveBeenCalledWith(
      InventoryActions.fetchSubCategories()
    );
  });
  it("should show error on fetch invnetories failure", () => {
    store = setupDefaultStore({
      inventories: {
        inventories: {
          entities: {},
      ...
      error: {
        [InventoryActions.FETCH_INVENTORIES_FINISHED]: {
          errors: [
            {
              error: "Undefined",
  ...
  it("should show loading spinner on fetch inventories", () => {
    store = setupDefaultStore({
      requesting: {
        [InventoryActions.FETCH_INVENTORIES]: true,
      },
      inventories: {
        inventories: {
          entities: {},
  ...
  it("should show inventory table with data", () => {
    renderWithProvider(<InventoryTable />, { store });
    expect(screen.getByText("NPK")).toBeInTheDocument();
    expect(screen.getByText("provider1")).toBeInTheDocument();
    expect(screen.getAllByText("5")[0]).toBeInTheDocument();
    expect(screen.getByText("Basil")).toBeInTheDocument();
    expect(screen.getByText("proev")).toBeInTheDocument();
    expect(screen.getAllByText("45")[0]).toBeInTheDocument();
  });

  it("should call select inventory action on row click", () => {
    renderWithProvider(<InventoryTable />, { store });
    const row = screen.getAllByRole("row")[1];
    row.click();
    expect(store.dispatch).toHaveBeenCalledWith(
      InventoryActions.selectInventory({
        inventoryId: "66b1bb23e33955b963f179a1",
        productId: "6626234c3c4db8c98290828e",
        name: "NPK",
        farmId: "fm1a215ac2",
  ...
  it("should route to add inventory page on add button click", () => {
    renderWithProvider(<InventoryTable />, { store });
    const addButton = screen.getByText("Add new");
    addButton.click();
    expect(mockedUsedNavigate).toHaveBeenCalledWith("/inventory/add");
  });

  it("should dispatch search filter on search ", async() => {
    renderWithProvider(<InventoryTable />, { store });
    const searchInput = screen.getByPlaceholderText("Search inventory");
    expect(searchInput).toBeInTheDocument();
    await userEvent.type(searchInput, "NPK");
    expect(store.dispatch).toHaveBeenCalledWith(InventoryActions.setFilter({ search: "NPK" }));
  })
  it("should dispatch category filter on category change", async () => {
    renderWithProvider(<InventoryTable />, { store });
    const categorySelect =
      screen.getByTestId("category-select").firstElementChild;
    if (categorySelect) {
      await userEvent.click(categorySelect);
      await userEvent.click(screen.getByText("Seeds"));
    }
    expect(store.dispatch).toHaveBeenCalledWith(
      InventoryActions.setFilter({
        category: "65dcc0591e82ab3fc6d9d768",
        product: null,
      })
    );
  });
  it("should dispatch category filter to all on category change", async () => {
    renderWithProvider(<InventoryTable />, { store });
    const categorySelect =
      screen.getByTestId("category-select").firstElementChild;
    if (categorySelect) {
  ...
  it("should dispatch product filter on product change", async () => {
    renderWithProvider(<InventoryTable />, { store });
    const productSelect =
      screen.getByTestId("product-select-filter").firstElementChild;
    if (productSelect) {
      await userEvent.click(productSelect);
      await userEvent.click(screen.getByText("All"));
    }
    expect(store.dispatch).toHaveBeenCalledWith(
      InventoryActions.setFilter({ product: null })
    );
  });
  it("should dispatch delete inventory action on delete button click", async() => {
    renderWithProvider(<InventoryTable />, { store });
    const checkboxes = screen.getAllByRole("checkbox");
    await userEvent.click(checkboxes[1]);
    const deleteButton = screen.getByTestId("delete-inventory-button");
  ...
  it("should render sidebar on inventory select", () => {
    store = setupDefaultStore({
      inventories: {
        inventories: {
          entities: {
