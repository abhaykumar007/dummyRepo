...
describe("Task List", () => {
    ...
    store = setupDefaultStore({
      users: {
        users: {
          ...
          result: ["61530dca-7031-70da-b1d5-db5d1ecf29c2"],
        },
        selectedUser: null,
      },
      inventories: {
  ...
  it("should call onSearch function", async () => {
    ...
    await waitFor(() => {
      expect(store.dispatch).toHaveBeenCalledWith(
        TaskActions.setFilters({ search: "test" })
      );
    });
  });
  ...
  it("should drag and drop task", async () => {
    renderWithProvider(<TaskList />, { store });
    const taskOne = screen.getByTestId("66aa1649806aafc4c874ecff");
    const columnOpen = screen.getByTestId("66a33ea4b431b975d7829ff9");

    expect(taskOne).toBeInTheDocument();
    expect(columnOpen).toBeInTheDocument();
    await waitFor(() => {
      fireEvent.dragStart(taskOne);
      fireEvent.dragEnter(columnOpen);
      fireEvent.dragOver(columnOpen);
      fireEvent.drop(columnOpen);
    ...
    //       { droppableId: "inProgress", index: 0 },
    //       "open"
    //     )
    //   );
    // });
  ...
  it("should call dispatch on user filter", async () => {
    renderWithProvider(<TaskList />, { store });
    const user = screen.getByTestId(
      "task-avatar-61530dca-7031-70da-b1d5-db5d1ecf29c2"
    );
    expect(user).toBeInTheDocument();
    fireEvent.click(user);
    await waitFor(() => {
      expect(store.dispatch).toHaveBeenCalledWith(
        TaskActions.setFilters({ user: "61530dca-7031-70da-b1d5-db5d1ecf29c2" })
      );
    });
  });

  it("should remove user filter", async () => {
    store = setupDefaultStore({
      users: {
        users: {
          entities: {
      ...
      tasks: {
        ...
        filters: {
          user: "61530dca-7031-70da-b1d5-db5d1ecf29c2",
          search: null,
        },
      },
    });
    ...
    await waitFor(() => {
      expect(store.dispatch).toHaveBeenCalledWith(
        TaskActions.setFilters({ user: null })
      );
    });
  });
});
