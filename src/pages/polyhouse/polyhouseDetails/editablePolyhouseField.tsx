...
interface EditablePolyhouseFieldProps {
  fieldName: string;
  value: string;
  placeholder?: string;
...
const EditablePolyhouseField = ({
  ...
  isParseField,
  type,
}: EditablePolyhouseFieldProps) => {
  const dispatch = useAppDispatch();
  const [form] = AntdForm.useForm();

  const selectedPolyhouse: Polyhouse | null = useAppSelector(
    PolyhouseSelectors.SelectSelectedPolyhouse
  );

  const loading = useAppSelector((state) =>
    requestingSelector(state, [PolyhouseActions.UPDATE_POLYHOUSES], fieldName)
  );

  const error = useAppSelector((state) =>
    selectError(state, PolyhouseActions.UPDATE_POLYHOUSES_FINISHED, fieldName)
  );
  ...
  const updatePolyhouse = <T extends { [key: string]: any }>(value: T) => {
    if (selectedPolyhouse) {
      dispatch(
        PolyhouseActions.updatePolyhouse(
          fieldName,
...
export default EditablePolyhouseField;
