...
interface EditableNurseryFieldProps {
  fieldName: string;
  value: string;
  placeholder?: string;
...
const EditableNurseryField = ({
  ...
  nurseryId,
  polyhouseId,
}: EditableNurseryFieldProps) => {
  const dispatch = useAppDispatch();
  const [form] = AntdForm.useForm();

  const loading = useAppSelector((state) =>
    requestingSelector(state, [PolyhouseActions.UPDATE_NURSERY], fieldName)
  );

  const error = useAppSelector((state) =>
    selectError(state, PolyhouseActions.UPDATE_NURSERY_FINISHED, fieldName)
  );
  ...
  const updateNursery = <T extends { [key: string]: any }>(value: T) => {
    dispatch(
      PolyhouseActions.updateNursery(fieldName, polyhouseId, nurseryId, {
        ...value,
      })
    );
  };
...
export default EditableNurseryField;
