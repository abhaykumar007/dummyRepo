...
const NurserySensor = () => {
  ...
  const sensorData = useAppSelector(PolyhouseSelectors.SelectSensorNurseryData);
  ...
  const requestData = () => {
    dispatch(
      PolyhouseActions.requestNurserySensorData(selectedNursery.nurseryId)
    );
  };

  useEffect(() => {
    if (selectedNursery) requestData();
  }, [selectedNursery]);
  ...
  const handleOnClick = (sensorName: string) => {
    ...
    if (param.polyhouseId && param.nurseryId)
      navigate(
      ...
      <Card
        ...
        title={
          <div className="heading1">
            {getTranslation("polyhouse.polyhouseDetails.sensors")}
          </div>
        }
        extra={
          <Flex gap={20}>
      ...
      >
        <div className="cursor_pointer">
          ...
          {!loading && !sensorData && !error && (
            ...
            >
              {getTranslation("polyhouse.polyhouseDetails.noSensors")}
            </span>
          )}
          {!loading && lastUpdated && (
            <div
